    ...
    try {
      ...
      const allLogs = await fetchAll<any>(() =>
        supabase
          ...
          .select(
            "trip_code,vehicle_id,fuel_expense,parking_charges,odometer_start,odometer_end,trip_date",
          )
          .gte("trip_date", start)
          .lt("trip_date", end),
      );
      ...
      const agg: Record<
        string,
        { fuel: number; parking: number; distance: number; trips: number }
      > = {};
      vehicles.forEach((v: any) => {
        agg[v.id] = { fuel: 0, parking: 0, distance: 0, trips: 0 };
      });
                      ...
                      <Fragment key={row.vehicle_id}>
                        <tr className="transition-colors hover:bg-muted/30">
                          ...
                          <td className="px-4 py-3 text-right text-orange-600 font-medium">
                            {row.total_fuel > 0 ? inr(row.total_fuel) : "—"}
                          </td>
                          <td className="px-4 py-3 text-right text-blue-600 font-medium">
                            {row.total_parking > 0 ? inr(row.total_parking) : "—"}
                          </td>
                          <td className="px-4 py-3 text-center">
                            <Button
                              ...
                              onClick={() =>
                                isExpanded ? setSelectedId(null) : loadHistory(row.vehicle_id)
                              }
                            >
                              {isExpanded ? (
                                ...
                                {loadingHistory ? (
                                  ...
                                  <table className="w-full text-xs">
                                    ...
                                    <tbody className="divide-y divide-border/50">
                                      {history.map((h: any) => {
                                        const dist =
                                          h.odometer_end != null && h.odometer_start != null
                                            ? Number(h.odometer_end) - Number(h.odometer_start)
                                            : null;
                                        return (
                                          <tr key={h.id}>
                                            ...
                                            <td className="py-2 text-right">
                                              {dist != null && dist > 0 ? `${dist} km` : "—"}
                                            </td>
                                            <td className="py-2 text-right text-orange-600">
                                              {Number(h.fuel_expense) > 0
                                                ? inr(Number(h.fuel_expense))
                                                : "—"}
                                            </td>
                                            <td className="py-2 text-right text-blue-600">
                                              {Number(h.parking_charges) > 0
                                                ? inr(Number(h.parking_charges))
                                                : "—"}
                                            </td>
                                          </tr>
                                        );
