    ...
    try {
      ...
      const allLogs = await fetchAll<any>(() =>
        supabase
          ...
          .select("trip_code,transporter_id,hire_charges,approval_charge,trip_date")
          .gte("trip_date", start)
          .lt("trip_date", end),
      );
      ...
      transporters.forEach((d: any) => {
        agg[d.id] = { hire: 0, approval: 0, trips: 0 };
      });
                ...
                <>
                        ...
                        <Fragment key={row.transporter_id}>
                          <tr className="transition-colors hover:bg-muted/30">
                            ...
                            <td className="px-4 py-3 text-right text-purple-600 font-medium">
                              {row.total_hire > 0 ? inr(row.total_hire) : "—"}
                            </td>
                            <td className="px-4 py-3 text-right text-emerald-600 font-medium">
                              {row.total_approval > 0 ? inr(row.total_approval) : "—"}
                            </td>
                                  ...
                                  {loadingHistory ? (
                                    ...
                                    <table className="w-full text-xs">
                                      ...
                                      <tbody className="divide-y divide-border/50">
                                        {history.map((h: any) => {
                                          const t =
                                            Number(h.approval_charge) - Number(h.hire_charges);
                                          return (
                                            <tr key={h.id}>
                                              <td className="py-2">{h.trip_date}</td>
                                              <td className="py-2 font-medium">{h.trip_code}</td>
                                              <td className="py-2 text-right text-purple-600">
                                                {Number(h.hire_charges) > 0
                                                  ? inr(Number(h.hire_charges))
                                                  : "—"}
                                              </td>
                                              <td className="py-2 text-right text-emerald-600">
                                                {Number(h.approval_charge) > 0
                                                  ? inr(Number(h.approval_charge))
                                                  : "—"}
                                              </td>
                  ...
                  <tr className="bg-muted/50 font-bold border-t-2 border-border">
                    ...
                    <td className="px-4 py-3 text-right">
                      {filtered
                        .filter((r) => r.trip_count > 0)
                        .reduce((acc, r) => acc + r.trip_count, 0)}
                    </td>
                    <td className="px-4 py-3 text-right text-purple-700">
                      {inr(
                        filtered
                          .filter((r) => r.trip_count > 0)
                          .reduce((acc, r) => acc + r.total_hire, 0),
                      )}
                    </td>
                    <td className="px-4 py-3 text-right text-emerald-700">
                      {inr(
                        filtered
                          .filter((r) => r.trip_count > 0)
                          .reduce((acc, r) => acc + r.total_approval, 0),
                      )}
                    </td>
                    <td className="px-4 py-3 text-right">
                      {inr(
                        filtered
                          .filter((r) => r.trip_count > 0)
                          .reduce((acc, r) => acc + (r.total_approval - r.total_hire), 0),
                      )}
                    </td>
