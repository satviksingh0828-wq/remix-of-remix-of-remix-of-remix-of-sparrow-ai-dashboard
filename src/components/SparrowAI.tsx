...
import { saveSparrowWorkspaceArtifact, type SparrowWorkspaceChartType } from "@/lib/sparrow-workspace";
...
Dashboard (/dashboard): admin-only stat cards, P&L, trip summary, entity P&L, AI WORKSPACE for SparrowAI visuals/tables, and branch/date/month/year filters.
...
function wantsWorkspacePresentation(text: string) {
  return /(workspace|visual|visuals|chart|graph|pie|bar|line|table|present|presentation|dashboard|show)/i.test(text) && /(expense|expenditure|income|trip|manifest|log|fastag|report|data)/i.test(text);
}

function inferWorkspaceChartType(text: string): SparrowWorkspaceChartType {
  const lower = text.toLowerCase();
  if (/pie|donut/.test(lower)) return "pie";
  if (/line|trend/.test(lower)) return "line";
  if (/area/.test(lower)) return "area";
  return "bar";
}

async function buildWorkspaceRows(text: string, role: string, branchIds: string[] | undefined) {
  const allowedBranchIds = role === "basic" ? (branchIds ?? []) : null;
  if (allowedBranchIds !== null && allowedBranchIds.length === 0) {
    throw new Error("Your account has no assigned branches, so I cannot present business data.");
  }
  ...
  if (kind === "incomes") {
    ...
    rows = await fetchAll<ReadOnlyRow>(() => withBranch(supabase.from("trips").select("id,trip_code,branch_id,ownership,start_date,start_time,vehicle_id,driver_id,transporter_id,created_at").order("created_at", { ascending: false })));
    title = "Open Trips Workspace";
  } else if (kind === "closed_trips") {
    rows = await fetchAll<ReadOnlyRow>(() => withBranch(supabase.from("closed_trips").select("id,trip_code,branch_id,branch_name,start_date,end_date,closed_at,total_income,total_expense,net_income,vehicle_id,driver_id,transporter_id").gte("closed_at", start).lt("closed_at", end).order("closed_at", { ascending: false })));
    title = "Closed Trips Workspace";
  } else if (kind === "manifests") {
    rows = await fetchAll<ReadOnlyRow>(() => supabase.from("trip_manifests").select("id,trip_id,source_id,manifest_number,weight,quantity,freight_amount,loading_amount,created_at").order("created_at", { ascending: false }));
    title = "Manifest Workspace";
  } else if (kind === "fastag") {
    rows = await safeFetchAll<ReadOnlyRow>(() => db.from("fastag_transactions").select("id,vehicle_id,amount,transaction_type,transaction_date,description,trip_code").gte("transaction_date", start).lt("transaction_date", end).order("transaction_date", { ascending: false }));
    title = "Fastag Workspace";
  } else if (kind === "logs") {
    if (role !== "admin") throw new Error("Audit and ledger workspaces are admin-only.");
    rows = await safeFetchAll<ReadOnlyRow>(() => db.from("app_logs").select("id,created_at,username,entity_type,action,entity_label,details").gte("created_at", start).lt("created_at", end).order("created_at", { ascending: false }));
    title = "Audit Logs Workspace";
  } else {
    rows = await fetchAll<ReadOnlyRow>(() => withBranch(supabase.from("expenditures").select("id,expenditure_name,amount,entry_date,paid_date,branch_id,vehicle_id,driver_id,transporter_id,is_insurance,is_road_tax,is_fastag_recharge,note").gte("entry_date", start).lt("entry_date", end).order("entry_date", { ascending: false })));
    title = "Expenditure Workspace";
  }
  ...
  return { rows, title, start, end, label, kind };
}

async function presentWorkspace(text: string, role: string, branchIds: string[] | undefined) {
  const result = await buildWorkspaceRows(text, role, branchIds);
  saveSparrowWorkspaceArtifact({
    id: uid(),
    title: result.title,
    description: `Generated by SparrowAI for ${result.start} to ${result.end}. Use chart buttons for bar, line, area or pie visuals and the table filter to search rows.`,
    createdAt: new Date().toISOString(),
    source: result.kind,
    chartType: inferWorkspaceChartType(text),
    rows: result.rows,
  });
  return `Done — I opened Dashboard → AI WORKSPACE and presented ${result.rows.length} row(s) as ${inferWorkspaceChartType(text)} visuals with a filterable table.`;
}
...
- If the user asks to present/show data as charts, visuals, pie/bar/line graph, or table workspace, open Dashboard → AI WORKSPACE and render it there.
...
- Dashboard, AI WORKSPACE, Reports → /dashboard, /reports (admin only)
...
export function SparrowAIPanel() {
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      try {
        if (wantsWorkspacePresentation(trimmed)) {
          const content = await presentWorkspace(trimmed, role, user?.branchIds);
    ...
    [loading, messages, role, displayName, currentPath, user?.branchIds, navigate],
  );
