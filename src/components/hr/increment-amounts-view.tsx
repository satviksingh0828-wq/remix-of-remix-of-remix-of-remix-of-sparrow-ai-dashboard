...
import { useCreateIncrementAmount, useDeleteIncrementAmount, useEmployees, useIncrementAmounts, useMarkIncrementPaid } from '@/lib/hr/hooks';
import { fullName } from '@/lib/hr/types';

const money = (n: number) => '₹' + (Number(n) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

export function IncrementAmountsView() {
  const { data: employees } = useEmployees();
  const { data: records, isLoading } = useIncrementAmounts();
  const create = useCreateIncrementAmount();
  const markPaid = useMarkIncrementPaid();
  const remove = useDeleteIncrementAmount();
  ...
  async function add() {
    ...
    try {
      ...
      setAmount(''); setReason(''); toast.success('Increment amount added.');
  ...
  function exportCsv() {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = 'increment-amounts.csv'; a.click(); URL.revokeObjectURL(a.href);
  }
  return <div className="mx-auto max-w-5xl space-y-4">
    <div><h1 className="text-xl font-bold sm:text-2xl">Increment amounts</h1><p className="text-sm text-muted-foreground">One-time additions applied to the employee's next generated payroll.</p></div>
    ...
    <div className="rounded-lg border bg-card"><div className="border-b px-4 py-3 text-sm font-semibold">Increment register</div>{isLoading ? <div className="p-4 text-sm text-muted-foreground">Loading…</div> : !records?.length ? <div className="p-4 text-sm text-muted-foreground">No increment amounts yet.</div> : <div className="divide-y">{records.map(r => <div key={r.id} className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm"><div><div className="font-medium">{employeeMap.get(r.employee_id) ? fullName(employeeMap.get(r.employee_id)!) : 'Unknown employee'}</div><div className="text-xs text-muted-foreground">{r.reason || 'No reason'} · {r.status}{r.payroll_id ? ` · payroll ${r.payroll_id.slice(0, 8)}` : ''}</div></div><div className="flex items-center gap-3"><span className="font-semibold">{money(r.amount)}</span>{r.status === 'pending' && <Button size="sm" variant="outline" onClick={() => markPaid.mutate(r.id)}><CheckCircle2 className="mr-1 h-3 w-3" />Mark handled</Button>}<Button size="icon" variant="ghost" onClick={() => remove.mutate(r.id)} aria-label="Delete increment"><Trash2 className="h-4 w-4" /></Button></div></div>)}</div>}</div>
  </div>;
}
