...
function dr(label: string, value: string): string {
  ...
  return `<div class="tn-detail-row"><span class="tn-detail-label">${htmlEsc(label)}</span><span class="tn-detail-value">${htmlEsc(val)}</span></div>`;
}
...
/* Internal expense table */
.tn-expense-section { margin-top: 10px; page-break-inside: avoid; }
.tn-expense-title { font-size: 10px; text-transform: uppercase; letter-spacing: .1em; color: #666; font-weight: 700; margin-bottom: 4px; }
.tn-expense-table { width: 100%; border-collapse: collapse; }
.tn-expense-table th { padding: 5px 7px; font-size: 9px; text-transform: uppercase; letter-spacing: .07em; font-weight: 700; background: ${primaryHex}; color: #fff; border: 1px solid ${primaryBorder}; }
.tn-expense-table td { padding: 4px 7px; border: 1px solid #ccc; font-size: 10px; vertical-align: top; }
.tn-expense-table tbody tr:nth-child(even) { background: #fafafa; }
.tn-expense-table tfoot td { border: 1px solid #bbb; padding: 4px 7px; background: #f0f0f0; font-weight: 700; }
...
function buildBodyHtml(
  ...
  const addrParts = [
    ...
    [sv((addr as Record<string, unknown>).city), sv((addr as Record<string, unknown>).state)]
      .filter(Boolean)
      .join(", "),
  ...
  const expenseRows = expenses.filter((expense) => sv(expense.name).trim() !== "");
  const expenseTotal = expenseRows.reduce((total, expense) => {
    const value = Number(sv(expense.amount).replace(/,/g, ""));
    return total + (Number.isFinite(value) ? value : 0);
  }, 0);
  const expenseBlock =
    documentTitle === "Internal Note"
      ? `
  <div class="tn-expense-section">
    <div class="tn-expense-title">Trip Expenses</div>
    <table class="tn-expense-table">
      <thead><tr><th class="tn-tc" style="width:36px">S.No.</th><th>Expense</th><th>Note</th><th class="tn-tr" style="width:100px">Amount</th></tr></thead>
      <tbody>
        ${
          expenseRows.length > 0
            ? expenseRows
                .map(
                  (expense, index) => `
          <tr>
            <td class="tn-tc">${index + 1}</td>
            <td>${htmlEsc(expense.name)}</td>
            <td>${htmlEsc(sv(expense.note) || "—")}</td>
            <td class="tn-tr">${htmlEsc(sv(expense.amount) || "—")}</td>
          </tr>`,
                )
                .join("")
            : `<tr><td colspan="4" class="tn-tc" style="padding:10px;color:#999">No expenses recorded for this trip.</td></tr>`
        }
      </tbody>
      <tfoot><tr><td colspan="3" class="tn-tr">Total Expenses</td><td class="tn-tr">${expenseRows.length > 0 ? expenseTotal.toFixed(2) : "—"}</td></tr></tfoot>
    </table>
  </div>`
      : "";
...
<div class="tn-root">
  ...
  <div class="tn-title">${htmlEsc(documentTitle)}</div>
  ...
  <div class="tn-company-header">
    ...
    <div class="tn-company-info">
      ...
      ${(() => {
        ...
        const gstin =
          sv((branch as Record<string, unknown> | null | undefined)?.gstin) || sv(company.gstin);
        const pan =
          sv((branch as Record<string, unknown> | null | undefined)?.pan) || sv(company.pan);
        return gstin || pan
 ...
 */
async function printTripDocument(
  data: TripNoteData,
  ...
  const filePrefix = documentTitle.replace(/\\s+/g, "");
...
<style>
  ...
  <div class="ring"></div><p>Generating ${documentTitle} PDF…</p>
</body></html>`);
    tab.document.close();
  }
  ...
  try {
    ...
    iframeDoc.write(
      `<!DOCTYPE html><html><head><meta charset="UTF-8">` +
        ...
        buildBodyHtml(data, logoDataUri, documentTitle, expenses) +
        `</body></html>`,
    );
    iframeDoc.close();
...
<title>${documentTitle} \u2014 ${tripCode}</title>
<style>
...
<div class="bar">
  <div>
    <div class="t1">${documentTitle}</div>
...
<script>
var _d="${pdfDataUri}";
function dl(){var a=document.createElement("a");a.href=_d;a.download="${filePrefix}-${tripCode}.pdf";a.click();}
</script>
</body></html>`;
    ...
    if (tab && !tab.closed) {
      ...
      pdf.save(`${filePrefix}-${tripCode}.pdf`);
    }
  } catch (err) {
