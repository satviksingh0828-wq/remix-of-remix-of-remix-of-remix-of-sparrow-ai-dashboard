...
const PAYROLL_CC = "priyanshi@garudalogistics.in";
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const employeeName = typeof body.employeeName === "string" ? body.employeeName.trim() : "Employee";
          const periodLabel = typeof body.periodLabel === "string" ? body.periodLabel.trim() : "the selected period";
          const filename = typeof body.filename === "string" && body.filename.endsWith(".pdf")
            ? body.filename
            : "payslip.pdf";
          const pdfBase64 = typeof body.pdfBase64 === "string" ? body.pdfBase64.replace(/\s/g, "") : "";

          if (!validEmail(to)) return Response.json({ error: "Employee email is invalid." }, { status: 400 });
