...
begin
  ...
  if v_token is null then
    ...
    on conflict (trip_id) do update set token = excluded.token, token_hash = excluded.token_hash, expires_at = null;
  else
    update public.driver_trip_qr_tokens set expires_at = null where trip_id = v_trip.id;
  end if;
...
declare
  ...
  v_link_id uuid;
begin
  if coalesce(trim(p_qr_token), '') = '' or coalesce(trim(p_device_id), '') = '' then raise exception 'QR token and device ID are required'; end if;
  select q.id, q.trip_id, t.trip_code, t.ownership into v_qr
  ...
  if found then
    v_link_id := v_link.id;
    update public.driver_trip_links set session_token_hash = encode(digest(v_session_token, 'sha256'), 'hex'), last_seen_at = now() where id = v_link_id;
  else
    insert into public.driver_trip_links (trip_id, device_id, session_token_hash, last_seen_at)
    values (v_qr.trip_id, p_device_id, encode(digest(v_session_token, 'sha256'), 'hex'), now()) returning id into v_link_id;
  end if;
